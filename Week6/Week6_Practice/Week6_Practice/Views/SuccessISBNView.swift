...
struct SuccessISBNView: View {
    ...
    private func bookInfoView() -> some View {
        VStack(spacing: 16, content: {
            ...
            makeInfoText(title: "저자", value: viewModel.bookModel?.authors.first ?? "저자가 없습니다.")
            makeInfoText(title: "장르", value: viewModel.bookModel?.contents ?? "장르가 없습니다.")
            makeInfoText(title: "가격", value: String(viewModel.bookModel?.price ?? 0))
            makeInfoText(title: "책소개", value: viewModel.bookModel?.thumbnail ?? "책소개가 없습니다.")
        })
    }
